...
const isCrudNotificationText = (text: string) => {
  ...
  // Extra safety for common RH CRUD phrases that still slip through.
...
export const getNotificationTarget = (notif: NotificationResponse): NotificationTarget => {
  ...
  return { kind: 'rh', path: '/dashboard' };
};

export const getRhNotificationPath = (notif: NotificationResponse) => {
  const target = getNotificationTarget(notif);
  if (target.kind === 'projects') return '/calendrier';
  return target.path;
};
